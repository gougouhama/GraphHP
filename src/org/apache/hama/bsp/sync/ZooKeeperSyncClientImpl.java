...
public class ZooKeeperSyncClientImpl extends ZKSyncClient implements
  ...
  public void init(Configuration conf, BSPJobID jobId, TaskAttemptID taskId)
    ...
    //quorumServers : hadoopx:21810
//    LOG.debug("quorumServers : "+quorumServers);
    ...
    //bspRoot : /bsp
    LOG.debug("bspRoot : "+bspRoot);
    /**
     * Constants.PEER_HOST和Constants.PEER_PORT 在GroomServer.BSPPeerChild类的main()方法里
     * 设置了值.如 hadoopx , 61001
     */
    String bindAddress = conf.get(Constants.PEER_HOST,
        Constants.DEFAULT_PEER_HOST);
    LOG.debug("bindAddress : "+bindAddress); // hadoopx
   
    //配置文件里定义了 bsp.peer.port=61000
    int bindPort = conf
        .getInt(Constants.PEER_PORT, Constants.DEFAULT_PEER_PORT);
    LOG.debug("bindPort : "+bindPort);	//61001
...
//    LOG.info("Start connecting to Zookeeper! At " + peerAddress);
  ...
  public void register(BSPJobID jobId, TaskAttemptID taskId,
      String hostAddress, long port) {
		/**
		 * jobId : job_201305310457_0002 
		 * taskId :attempt_201305310457_0002_000000_0 
		 * hostAddress : hadoopx 
		 * port : 61002
		 */
...
//	  LOG.debug("port : "+port);
    try {
      ...
      //jobRegisterKey : /bsp/job_201305310457_0002/peers
      LOG.debug("jobRegisterKey : "+jobRegisterKey);
      
      if (zk.exists(jobRegisterKey, false) == null) {
    	  //创建不成功，发生KeeperException$NoNodeException: KeeperErrorCode = NoNode 异常.
    	  //因为不存在 /bsp/job_201305310457_0002
  ...
  public void registerTask(BSPJobID jobId, String hostAddress, long port,
    ...
    String taskRegisterKey = constructKey(jobId, "peers", hostAddress + ":"
        + port);
    //taskRegisterKey : /bsp/job_201305310457_0003/peers/hadoopx:61002
    LOG.debug("taskRegisterKey : "+taskRegisterKey);
