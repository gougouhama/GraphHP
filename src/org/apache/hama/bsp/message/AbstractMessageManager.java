...
//		LOG.info("peerAddress : "+peerAddress);
		...
		//结果: org.apache.hama.bsp.message.queue.MemoryQueue
//		LOG.info("localQueue( 类型 MessageQueue<M>) : "+localQueue);
		this.localQueueForNextIteration = getSynchronizedQueue();	//也会调用 getQueue()方法
		//结果: org.apache.hama.bsp.message.queue.SingleLockQueue@1736a73
//		LOG.info("localQueueForNextIteration( 类型 SynchronizedQueue<M><M>) : "+localQueueForNextIteration);
	...
	public void finishSendPhase() throws IOException {
		//LOG.info("outgoingQueues(类型 HashMap<InetSocketAddress, MessageQueue<M>>) : "+outgoingQueues);
		Collection<MessageQueue<M>> values = outgoingQueues.values();
		for (MessageQueue<M> msgQueue : values) {
			//LOG.info("msgQueue(类型 MessageQueue) : "+msgQueue);
	 ...
	 * nothing has been configured for "hama.messenger.queue.class" then the
	...
	protected MessageQueue<M> getQueue() {
		Class<?> queueClass = conf
				.getClass(QUEUE_TYPE_CLASS, MemoryQueue.class);
		...
		//MessageQueue对象 : org.apache.hama.bsp.message.queue.MemoryQueue@b43967
		//LOG.info("MessageQueue对象 : "+newInstance);
