...
/**
 * value = in.readInt(); A message that is either MapWritable (for meta
 * communication purposes) or a real message (vertex ID and value). It can be
 * extended by adding flags, for example for a graph repair call.
 */
public final class GraphJobMessage implements Writable {
	public static final Log LOG = LogFactory.getLog(GraphJobMessage.class);
	...
	public static final int BOUNDARY_VERTEX_SIZE_FLAG=0x20;
	...
	private IntWritable boundaryVertex_size;
	...
	public GraphJobMessage(IntWritable size,int sign) {
		this.flag = BOUNDARY_VERTEX_SIZE_FLAG;
		this.boundaryVertex_size = size;
	}
	...
	public void write(DataOutput out) throws IOException {
		...
		if (isVertexMessage()) {
			// we don't need to write the classes because the other side has the
			// same
			...
			//LOG.info("－－－－－－加载数据发送分区信息－－－－－－");
			//LOG.info("vertex.getVertexID() : "+ vertex.getVertexID().toString());
			vertex.getVertexID().write(out);
			//LOG.info("vertex.getValue() : " + vertex.getValue());
			if (vertex.getValue() != null) {
				//LOG.info("写入vertex的value");
			...
			for (Object e : outEdges) {
				...
				if (edge.getValue() != null) {
					...
					//LOG.info("出边没有值,即dge.getValue()==null");
					out.writeBoolean(false);
				}
			}
		} else if (isVerticesSizeMessage()) {
			vertices_size.write(out);
		} else if (isBoundaryVertexSizeMessage()) {
			boundaryVertex_size.write(out);
		} else {
			vertexId.write(out);
		}
	...
	public void readFields(DataInput in) throws IOException {
		...
		if (isVertexMessage()) {
			...
			//LOG.info("再次调用反射机制创建 vertex");
			Vertex<Writable, Writable, Writable> vertex = GraphJobRunner
					.newVertexInstance(VERTEX_CLASS, null);
			Writable vertexId = ReflectionUtils.newInstance(VERTEX_ID_CLASS,
					null);
			vertexId.readFields(in);
			//LOG.info("收到的顶点 ： " + vertexId.toString());
			vertex.setVertexID(vertexId);
			if (in.readBoolean()) {
				Writable vertexValue = ReflectionUtils.newInstance(
						VERTEX_VALUE_CLASS, null);
				vertexValue.readFields(in);
				//LOG.info("收到顶点的值：" + vertexValue);
				vertex.setValue(vertexValue);
			}
			int size = in.readInt();
			//LOG.info("收到的出边数目大小: " + size);
			vertex.setEdges(new ArrayList<Edge<Writable, Writable>>(size));
			for (int i = 0; i < size; i++) {
				Writable edgeVertexID = ReflectionUtils.newInstance(
						VERTEX_ID_CLASS, null);
				edgeVertexID.readFields(in);
				//LOG.info("收到的出边顶点 ： " + edgeVertexID);
				Writable edgeValue = null;
				if (in.readBoolean()) {
					edgeValue = ReflectionUtils.newInstance(EDGE_VALUE_CLASS,
							null);
					edgeValue.readFields(in);
				}
				//LOG.info("收到的出边的值: " + edgeValue);
				vertex.getEdges().add(
						new Edge<Writable, Writable>(edgeVertexID, edgeValue));
			}
			this.vertex = vertex;
			//LOG.info("收到的顶点（Vertex）" + vertex);
		} else if (isVerticesSizeMessage()) {
			vertices_size = new IntWritable();
			vertices_size.readFields(in);
		} else if(isBoundaryVertexSizeMessage()) {
			boundaryVertex_size=new IntWritable();
			boundaryVertex_size.readFields(in);
		} else {
			vertexId = ReflectionUtils.newInstance(VERTEX_ID_CLASS, null);
			vertexId.readFields(in);
		}
	...
	/**
	 * @return the boundaryVertex_size
	 */
	public IntWritable getBoundaryVertexSize() {
		return boundaryVertex_size;
	}
	...
	public boolean isBoundaryVertexSizeMessage() {
		return flag == BOUNDARY_VERTEX_SIZE_FLAG;
	}

	@Override
	public String toString() {
		return "GraphJobMessage [flag=" + flag + ", map=" + map + ", vertexId="
				+ vertexId + ", vertexValue=" + vertexValue + ", vertex="
				+ vertex + ", vertices_size=" + vertices_size
				+ ", boundaryVertex_size=" + boundaryVertex_size + "]";
	}
}
