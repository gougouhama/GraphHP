...
public final class KMeansBSP
  ...
  public static void main(String[] args) throws IOException,
	  ...
	  //parameter: kmeans-In kmeans-out 100 5 10 100 3
    if (args.length < 6) {
    ...
    Path center = new Path("center/cen.seq");
    Path centerOut = new Path("center/center_output.seq");
    ...
    BSPJob job = createJob(conf, in, out, true);
   ...
   */
	public static void prepareInputText(int k, Configuration conf, Path in,
			Path center, Path out, FileSystem fs) throws IOException {

		if (fs.exists(out))
			fs.delete(out, true);
		if (fs.exists(center))
			fs.delete(new Path("center"), true);
		if (fs.exists(in))
			fs.delete(in, true);
		
		Path inFile = new Path(in, "in.seq");
		final NullWritable value = NullWritable.get();
		...
		final SequenceFile.Writer dataWriter = SequenceFile.createWriter(fs,
				conf, inFile, VectorWritable.class, NullWritable.class,
				CompressionType.NONE);

		int i = 0;
		Path txtIn = new Path("kmeansdata/clustering data");
  ...
  public static void prepareInput(int count, int k, int dimension,
    ...
    final SequenceFile.Writer dataWriter = SequenceFile.createWriter(fs, conf,
    		inFile, VectorWritable.class, NullWritable.class, CompressionType.NONE);
